...
public class CocktailControllerTest {
    ...
    void testSearch() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/cocktails").param("search", "Russian"))
                ...
                .andExpect(
                        MockMvcResultMatchers.jsonPath("$[?(@.name empty false)]", Matchers.hasSize(expectedSize)))
                .andExpect(
                        MockMvcResultMatchers.jsonPath("$[?(@.cocktailId empty false)]",
                                Matchers.hasSize(expectedSize)))
                .andExpect(
                        MockMvcResultMatchers.jsonPath("$[?(@.glass empty false)]", Matchers.hasSize(expectedSize)))
                .andExpect(
                        MockMvcResultMatchers.jsonPath("$[?(@.instructions empty false)]",
                                Matchers.hasSize(expectedSize)))
                .andExpect(
                        MockMvcResultMatchers.jsonPath("$[?(@.image empty false)]", Matchers.hasSize(expectedSize)))
                .andExpect(
